...
Plain-language definitions of the core concepts in warp. For *who is allowed to
do what*, see [PERMISSIONS.md](PERMISSIONS.md).
...
> book** that seat. Plans are *layout*, zones are *access control* — two
...
display **name**, and an **account type**. Account type is the *global* role:
...
### Site administrator
A user whose account type is *administrator*. Site admins are the only ones who
...
as a special account that **cannot sign in**. Assigning a *group* to a zone
...
#### Zone type
How *open* the zone is to people with no explicit role in it:
...
| **Public (view)** | Everybody may *see* the seats (but not book without a role). |
| **Public (book)** | Everybody may *see and book* the seats. |
...
most **one** seat across *all* zones in that group at the same time (handy for
...
(Distinct from a *disabled zone*, which turns off a whole group of seats.)
...
user (or *everyone*) and an optional **days-in-advance** value. If a seat has
any assignment, only the named users (or everyone, if there is an everyone row)
may book it. Assignment is *not* the same as a booking — it is a standing
reservation of the *right to book*.
...
*everyone* assignment to control when a reserved seat opens to the public.

### Release time
The moment a privately-assigned seat **opens for general booking**. If a seat is
assigned to Alice (unlimited) and also to *everyone* with 3 days-in-advance,
then for any given day the seat is Alice's until 3 days before — at that point it
"releases" and anyone may grab it. The calendar integration can send a *seat
release reminder* so the owner knows before their seat opens up.
...
release window, and existing bookings. Zone admins can auto-book *as* another
user, confined to the zones they administer.

### Effective zone role
The role a user *actually* has in a zone once everything is combined: their
